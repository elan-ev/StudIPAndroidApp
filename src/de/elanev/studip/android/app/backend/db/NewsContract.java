...
public final class NewsContract extends AbstractContract {
	...
	public static final String CREATE_STRING = String
			.format("create table if not exists %s (%s integer primary key, %s text unique,"
					...
					TABLE, BaseColumns._ID, Columns.NEWS_ID,
					Columns.NEWS_TOPIC, Columns.NEWS_BODY, Columns.NEWS_DATE,
					Columns.NEWS_USER_ID, Columns.NEWS_CHDATE,
					Columns.NEWS_MKDATE, Columns.NEWS_EXPIRE,
					Columns.NEWS_ALLOW_COMMENTS, Columns.NEWS_CHDATE_UID,
					Columns.NEWS_BODY_ORIGINAL, Columns.NEWS_COURSE_ID);
	public static final String NEWS_JOIN_USER = String.format(
			"%s INNER JOIN %s ON %s = %s", TABLE, UsersContract.TABLE,
			Qualified.NEWS_NEWS_USER_ID, UsersContract.Qualified.USERS_USER_ID);
	...
	public static final Uri GLOBAL_CONTENT_URI = BASE_CONTENT_URI.buildUpon()
			.appendPath(PATH).appendPath("global").build();
	public static final Uri COURSES_CONTENT_URI = BASE_CONTENT_URI.buildUpon()
			.appendPath(PATH).appendPath("courses").build();
	...
	public static final class Columns implements BaseColumns {
		...
		public static final String NEWS_COURSE_ID = "course_id";
	...
	public interface Qualified {
		...
		public static final String NEWS_NEWS_COURSE_ID = TABLE + "."
				+ Columns.NEWS_COURSE_ID;
	}
